...
exports.build_svg_from_score = build_svg_from_score;
...
function create_sq(svg, props) {
  ...
  sq.attr('x', props.x).attr('y', props.y).attr('width', props.size).attr('height', props.size).attr('stroke', '#ffffff').attr('stroke-width', props.strokeSize).attr('fill', props.fillColor);

  if (props.tooltip !== undefined) {
    sq.attr('class', 'insignia-tooltip').attr('data-tippy-delay', '0').attr('data-tippy-size', 'large').attr('data-tippy-placement', 'right').attr('data-container', 'body').attr('role', 'tooltip').attr('tabindex', -1).attr('title', props.tooltip);
  }
...
function build_svg(container, scores, settings) {
  ...
  var color = settings.color !== undefined ? settings.color : d3.scaleLinear().domain([0, 1]).interpolate(d3.interpolateRgb).range([d3.rgb(255, 0, 0), d3.rgb(0, 0, 255)]);
  var svg = settings.svg !== undefined ? settings.svg : d3.select(container).append('svg').attr('width', '100%').attr('height', '100%').attr('preserveAspectRatio', 'xMinYMin').attr('viewBox', '0 0 1 1');
  ...
  if (tooltips !== undefined) {
    (0, _tippy.default)('.insignia-tooltip');
  }
}

function build_svg_from_score(container, params) {
  var client = new _coreapi.default.Client();
  return client.action(_schema.default, ['score', 'list'], params).then(function (results) {
    build_svg(container, results.scores, {
      tooltips: function tooltips(rubric, metric, score) {
        return (isNaN(score) ? '' : 'Score: ' + (score * 100).toFixed(0) + '%<br />') + results.metrics[metric];
      },
      links: function links(rubric, metric, score) {
